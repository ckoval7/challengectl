...
For viewing agent statistics and the transmission feed, see the [Dashboard](Web-Interface-Dashboard) guide. For managing challenges assigned to runners, consult the [Challenge Management](Web-Interface-Challenges) guide. To view spectrum recordings captured by listeners, see the [Recordings Web Interface](Web-Interface-Recordings) guide. To view agent log output for troubleshooting, see the [Logs](Web-Interface-Logs) guide. For instructions on configuring and deploying runner agents, review the [Runner Setup](Runner-Setup) guide. For configuring and deploying listener agents, see the [Listener Setup](Listener-Setup) guide. To pause the system and stop task assignment, refer to the [System Controls](Web-Interface-System-Controls) guide.
